...
@Component({
  name: 'ShapeByValueConfigurator',
  components: {
    ByValueIconList
  }
})
export default class ShapeByValueConfigurator extends Mixins<ByValueMixin<IconClause>>(
  ByValueMixin
) {
  ...
  get shapeOptions(): IconMapping {
    return MAPPED_ICONS.shapes;
  }

  get firstShape() {
    return Object.keys(this.shapeOptions)[0];
  }
  ...
  resetIcons() {
    this.icons = this.icons.map(icon => [icon[0], icon[1] ? icon[1] : this.firstShape]);
  }
