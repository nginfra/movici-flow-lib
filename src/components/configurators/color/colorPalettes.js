...
export default class ColorPalette {
    constructor({ name, colors, colorsForSize, reversed }) {
        this.name = name;
        this.colors = colors;
        this.colorsForSize = colorsForSize;
        this.reversed = reversed !== null && reversed !== void 0 ? reversed : false;
    }
    getHexColorsForSize(size) {
        const rv = this.colorsForSize[size].map((n) => this.colors[n]);
        if (this.reversed) {
            rv.reverse();
        }
        return rv;
    }
    getColorTriplesForSize(size) {
        return this.getHexColorsForSize(size).map(color => hexToColorTriple(color));
    }
}
function addReversedColorPalettes(palettes) {
